...
public class GuideUtil {
    ...
    private static final ItemStack CER_MENU_BUTTON =
        Converter.getItem(new SlimefunItemStack(
            ...
            "&e&lCost Comparison (Reference Only)"
        ));
    ...
    public boolean checkRTS(Player pl) {
        try {
            ...
            if (!RTSSearchGroup.isRtsAvailable()) {
                ...
                pl.sendMessage(ChatColors.color("&cReal-time search is unavailable on server version " + MinecraftVersion.current().humanize() + ". The real-time-search library currently supports up to " + maxVersion.humanize()));
                return false;
            }
        } catch (Exception e) {
            Debug.trace(e);
            pl.sendMessage(ChatColors.color("&cCould not validate real-time search. The feature has been disabled."));
            return false;
        }
    ...
    public static ItemStack getLeftActionIcon(BaseAction action) {
        return Converter.getItem(
            action.material(),
            ChatColors.color("&7Press " + action.getKey().getKey() + " (" + action.name() + ")")
        );
    }
        ...
        for (int s : format.getChars(Formats.Char.SETTINGS_PANEL_BUTTON)) {
            ...
            menu.addMenuClickHandler(s, (pl, slot, item, action) -> EventUtil.callEvent(new GuideEvents.SettingsButtonClickEvent(pl, item, slot, action, menu, getLastGuide(pl))).ifSuccess(() -> {
                if (GroupResorter.isSelecting(pl)) {
                    pl.sendMessage(ChatColors.color("&cThe settings menu is unavailable in the current guide mode."));
                    return false;
                }
